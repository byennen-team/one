...
(function(window, document, $, undefined){
                                  ...
                                  placeholder_url, max_imagesize, onImageUpload, onKeyEnter )
    {
        ...
        var content_insertimage = function(wysiwygeditor)
        {
            ...
            if( window.File && window.FileReader && window.FileList && 1===0)
            {
                // File-API
                ...
                onselection: function( collapsed, rect, nodes, rightclick )
                    {
                        ...
                        // No selection-toolbar wanted?
                            ...
                            show_toolbar = false;
                        // Selected toolbar wanted, but
                        // nothing selected (=selection collapsed)
                        else if( rect === undefined || collapsed )
                            show_toolbar = false;
                        // Only one image? Better: Display a
                        // special image-toolbar
