  ...
  $.fn.mediumInsert.registerAddon('images', {
      ...
      */
      uploadFile: function ($placeholder, file, that) {
        var options = {
          ...
          onProgress: that.updateProgressBar,
    ...
    */
    ...
    uploadCompleted: function (jqxhr, $placeholder) {
      ...
      if (jqxhr.filePath) {
        var fullPath = Meteor.settings.public.AWS_BUCKET_URL +
          '/' + jqxhr.filePath;
        $progress.before('<figure class="mediumInsert-images"><img src="'+
          fullPath +'" draggable="true" alt=""></figure>');
        ...
        $progress.before('<div class="mediumInsert-error">There was ' +
          ' a problem uploading the file.</div>');
      ...
      $placeholder.closest('[data-medium-element]')
        .trigger('keyup').trigger('input');
    },
    ...
    */
    ...
    /**
    * Lopp though files, check file types and call uploadFile()
    * function on each file that passes
    *
    * @param {element} placeholder Placeholder to add image to
    ...
    uploadFiles: function ($placeholder, files, that) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (acceptedTypes[file.type] === true) {
          $placeholder
            .append('<progress class="progress" min="0" max="100"' +
              ' value="0">0</progress>');
